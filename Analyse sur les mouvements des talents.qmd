...
```{r}
here::i_am("Semiconductor-project.Rproj")
library(here)
library(readxl)
library(dplyr)
library(ggplot2)
...
model2_1 <- plm(
  formula = yield_rate  ~ inflow_count,  
  data = panel_data,
  index = c("country", "year"),
  model = "within"
)
...
model2_2 <- plm(
  formula = yield_rate ~ outflow_count,  
  data = panel_data,
  index = c("country", "year"),
  model = "within"
)
...
model2_2lag1 <- plm(
  yield_rate ~ lag(outflow_count, 1),   
  data = panel_data,
  index = c("country", "year"),
  model = "within"
)
...
model2_2lag2 <- plm(
  yield_rate ~ lag(outflow_count, 2),   
  data = panel_data,
  index = c("country", "year"),
  model = "within"
)
...
model2_2lag3 <- plm(
  yield_rate ~ lag(outflow_count, 3),   
  data = panel_data,
  index = c("country", "year"),
  model = "within"
)
...
model2_2lag4 <- plm(
  yield_rate ~ lag(outflow_count, 4),   
  data = panel_data,
  index = c("country", "year"),
  model = "within"
)
...
model2_2lag5 <- plm(
  yield_rate ~ lag(outflow_count, 5),   
  data = panel_data,
  index = c("country", "year"),
  model = "within"
)
...
#  Régressions linéaires séparées par pays pour estimer l'effet de net_flow sur nanometer
dat <- panel_data |>
  select(country, year, nanometer, net_flow) |>
  filter(!is.na(nanometer), !is.na(net_flow))

keep_countries <- dat |> 
  count(country) |> 
  filter(n >= 3) |> 
  pull(country)
dat <- dat |> filter(country %in% keep_countries)

by_country_results <- dat |>
  group_by(country) |>
  nest() |>
  mutate(
    model = map(data, ~ lm(nanometer ~ net_flow, data = .x)),
    tidy  = map(model, broom::tidy),    
    glance= map(model, broom::glance)   
  )

coef_table <- by_country_results |>
  unnest(tidy) |>
  filter(term == "net_flow") |>
  select(country,
         estimate, std.error, statistic, p.value) |>
  left_join(
    by_country_results |> unnest(glance) |> select(country, r.squared, adj.r.squared, n = df.residual),
    by = "country"
  ) |>
  arrange(p.value)
