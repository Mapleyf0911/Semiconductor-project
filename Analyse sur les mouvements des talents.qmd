...
```{r}
usethis::use_git()
here::i_am("Semiconductor-database.Rproj")
library(here)
library(readxl)
library(dplyr)
library(ggplot2)
...
move_with_exit_year <- data.frame(
  from = character(),
  to = character(),
  exit_year = character(),
  count = numeric(),
...
for(i in 1:nrow(migration_data)) {
  ...
  for(j in 1:6) {
    if(!is.na(countries[j]) && !is.na(countries[j+1]) && 
      ...
      move_with_exit_year <- rbind(move_with_exit_year, new_move)
...
yearly_counts <- move_with_exit_year |>
  group_by(exit_year) |>
...
ggplot(yearly_counts_filtered, aes(x = exit_year, y = total_count)) +
  ...
  labs(title = "Histogram of Talent Movements per Year",
       x = "Exit Year", y = "Total Count") +
  theme_minimal()


flow_matrix <- move_with_exit_year |>
  group_by(from, to) |>
...
move_from_2015 <- move_with_exit_year |>
...
```{r}
# Régression entre mouvements des talents et progrès des technologies de fabrication des semi-conducteurs
